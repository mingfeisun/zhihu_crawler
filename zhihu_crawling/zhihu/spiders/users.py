...
class UsersSpider(scrapy.Spider):
    ...
    def __init__(self, url = None):
        self.user_url = "https://www.zhihu.com/people/charles-wang-2012"
    ...
    def user_start(self, response):
        sel_root = response.css("div.List-item")

        # 判断关注列表是否为空
        if len(sel_root):
            for sel in sel_root:
                people_url = sel.css("a.UserLink-link::attr('href')").extract_first()
                ...
                yield scrapy.Request(
                    url = self.domain + people_url + '/followees',
                    headers = self.headers,
                    meta = {
                        'proxy': UsersConfig['proxy'],
                        'cookiejar': response.meta['cookiejar'],
                ...
                yield scrapy.Request(
                    url = self.domain + people_url + '/followers',
                    headers = self.headers,
                    meta = {
                        'proxy': UsersConfig['proxy'],
                        'cookiejar': response.meta['cookiejar'],
